...
| Discord 어댑터 | 이벤트 정규화, 참여 ID 기록, 활성·보관 쓰레드 열거, reaction 수명주기, 모든 히스토리 문맥 경로에서 플러그인이 제공한 범용 제외 판정 적용 | 관심 필요 여부 추론, 제어 명령 문법·장부 소유 |
...
| `pre_gateway_dispatch` 훅 | 한/영 키 정규화, 제어 명령 인식, 권한 확인, 단락 처리, 처리 결과 reaction 전환 | 일반 대화 추론 |
...
| 향후 검토 작업 | 두 후보군 중복 제거, 3일 재노출 제한, 5+5 가변 배분, 신규 3·과거 2 기본 배분, 메타데이터 기반 한 줄 항목과 초과 개수 생성 | keepalive, 메시지 본문·전수 내용 요약 |
...
`pre_gateway_dispatch`는 일반 게이트웨이 인증보다 먼저 실행된다. 따라서 플러그인
자체가 같은 허용 사용자·채널 정책을 확인한 뒤에만 장부를 바꿔야 한다. 장부에는
토큰, 메시지 본문, 대화 내용이 아닌 최소 식별자와 상태만 둔다.
...
- Discord 어댑터 변경은 플러그인별 명령을 알지 못하는 범용 히스토리 제외 연결부로
  제한하고 그 코어 테스트는 Hermes Agent worktree가 소유한다. 플러그인이 어댑터의
  비공개 필드나 메서드를 monkeypatch하지 않는다.
...
- 플러그인과 코어 변경은 각자 소유 저장소에서 테스트하고, 함께 필요한 변경은 두
  ...
  Discord를 변경하지 않는다.
- 기존 라이브 플러그인은 아직 자동 인벤토리나 재알림을 구현하지 않았다. 정확한
  코드·DB 백업, 설치와 롤백 순서는 [DEPLOYMENT.md](DEPLOYMENT.md)를 따른다.
