...
설치하고 실패 시 복구하는 절차의 권위다. 문서 합의만 진행하는 동안에는 이 절차를
실행하지 않는다.

## 배포 단위

- 플러그인 배포는 깨끗한 `agent-extensions` 작업 트리의 커밋된 리비전에서만 만든다.
  범용 Discord 연결부가 필요한 버전은 깨끗한
  `feat/discord-predispatch-thread-routing` Hermes Agent worktree의 커밋도 함께
  고정한다. 미커밋 파일이나 라이브 파일을 배포 원본으로 사용하지 않는다.
- 배포 묶음에는 플러그인 Git 커밋 SHA와 필요한 Hermes Agent Git 커밋 SHA, 생성 UTC
  시각, 설치 대상 상대 경로, 각 파일의 SHA-256, 플러그인 파일 스냅샷과 최소 Hermes
  Agent 패치를 담은 manifest를 둔다.
- 토큰, 사용자 ID, `config.yaml`, SQLite DB와 비공개 메시지는 배포 묶음에 넣지
  않는다.
- Discord 어댑터 전체 파일을 라이브에 무조건 덮어쓰지 않는다. 최소 패치에 대해
  라이브 소스에서 `git apply --check`에 해당하는 사전 검사를 하고, 적용할 수 없으면
  배포를 중단한다. 기존 라이브 변경을 자동 폐기하거나 우회하지 않는다.
...
- 설치가 바꿀 라이브 Hermes Agent 파일의 원본과 SHA-256
...
1. `agent-extensions` 작업 트리와 필요한 Hermes Agent 전용 worktree가 깨끗하고 각
   대상 커밋이 존재하는지 확인한다.
2. 각 저장소의 단위·통합 테스트와 비라이브 DB 마이그레이션 테스트를 통과시킨다.
3. 배포 묶음을 만들고 manifest의 파일·패치 해시를 다시 검증한다.
4. 라이브 대상, 디스크 여유, 게이트웨이 제어 방법과 최소 패치 적용 가능성을 읽기
   전용으로 검사한다. 하나라도 실패하면 아무것도 변경하지 않는다.
5. Hermes 게이트웨이를 정상 종료하고 Discord 쓰기와 SQLite writer가 멈췄는지
   확인한다.
6. SQLite WAL을 checkpoint하고 `quick_check`를 실행한 뒤 코드·설정·DB 백업과 백업
   manifest를 만들고 검증한다.
7. 플러그인 코드는 같은 파일시스템의 임시 sibling 디렉토리에 준비해 해시를 확인한
   뒤, 게이트웨이가 멈춘 상태에서 manifest에 적힌 대상 파일만 원자적 파일 교체로
   설치한다. Hermes Agent 변경은 검증한 최소 패치만 적용한다.
8. 추가 전용 DB 마이그레이션을 한 트랜잭션으로 실행하고 `quick_check`를 다시
   확인한다.
9. 실제 활성화 배포라면 다른 설정을 보존한 채 합의된 `thread_attention` 블록을
   병합하고, 저장한 YAML을 다시 읽어 정적 설정을 검증한다.
10. 게이트웨이를 시작하고 프로세스 상태, 플러그인 로드, DB schema version과
    다이제스트 채널 조회·전송 권한 검증 결과를 확인한다.
11. 첫 활성화라면 활성화 ID와 과거 가져오기 스냅샷이 한 번만 만들어졌는지,
    실시간 수집이 먼저 열린 뒤 batch가 진행되는지 확인한다. 완료 후 최초 요약
    전달 의무와 다이제스트 scheduler가 각각 한 번만 열렸는지 확인한다.
12. 지정한 폐기 가능 Discord 테스트 쓰레드에서 제어 명령 단락, 고정 확인,
    reaction, 상태 기록·취소와 LLM 미호출을 확인한다. 다이제스트 전송은 설정된 테스트
    대상에서 별도로 확인한다.
13. 초기화 중단·재시작 시험에서 같은 활성화 ID로 미완료 batch만 이어지고 요약과
    같은 날짜 다이제스트가 중복 생성되지 않는지 확인한다.
14. 성공 결과는 실제 사용자 ID와 메시지 본문을 제거해 `evidence/`에 남긴다.
